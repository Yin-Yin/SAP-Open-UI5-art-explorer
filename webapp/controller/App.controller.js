sap.ui.define([
	...
	return Controller.extend('sap.ui.demo.todo.controller.App', {
		...
		// _____________________________________________________________________________________________________________________________________________________
		// _____________________________________________________________________________________________________________________________________________________
		// not anymore used code


		toggleSubheader: function () {
			var subheader = this.byId("subHeader");
			subheader.setVisible(!subheader.getVisible());
		},


		fillImgOverviewItemsArrayOld: function () { //rename function!!
			var items = [];
			var paintingsArray = this.getView().getModel().oData.paintingDataCurrentStyle.Paintings;
			console.log("paintingsArray: ", paintingsArray);
			for (var i = 0; i < paintingsArray.length; i++) {
				items.push(new sap.m.StandardListItem({title: paintingsArray[i].artistName + " - " + paintingsArray[i].title}))
			}
			return items;
		},

		fillHistoryListItemArrayOld: function () {
			var items = [];
			var paintingHistory = this.getView().getModel().oData.paintingHistory;
			for (var i = 0; i < paintingHistory.length; i++) {
				var countHistory = i + 1;
				items.push(new sap.m.StandardListItem({title: countHistory + ". " + paintingHistory[i].artistName + " - " + paintingHistory[i].title}))
			}
			return items;
		},


		changePaintingStyle2: function () {
			var comboBox = this.byId("comboBoxPaintingStyle");
			var changeToPaintingStyle = comboBox.getSelectedKey();
			console.log("changeToPaintingStyle ", changeToPaintingStyle);
			this.loadPaintingStyleData(changeToPaintingStyle, false); // store the boolean value of featured or not to get the featured selection or not
			//this.triggerPaintingStyleDataLoad(changeToPaintingStyle, false); // store the boolean value of featured or not to get the featured selection or not
		},


		loadPaintingStyles: function () {
			var oModel = this.getView().getModel();
			var url = "https://cors.io/?http://www.wikiart.org/en/App/wiki/DictionariesJson/1"; // use https://cors.io/ to circumvent No 'Access-Control-Allow-Origin' header is present on the requested resource problem
			$.ajax({
				dataType: 'json',
				url: url,
				type: 'GET',
				...
				success: function (data) {
					...
					jQuery.sap.log.info('Successfully retrieved data');
					oModel.setProperty("/paintingStylesData", data);
					var styleArray = [];
					data.forEach((data) => {
							styleArray.push(data.title);
						}
					);
					oModel.setProperty("/paintingStylesArray", styleArray);
					console.log("oData after loadPaintingStyles", oModel.oData);
					this.changeToRandomPicture();
				},
				error: function (error) {
					jQuery.sap.log.error('Error on retrieving ressource');
				}
			});

		},

		fillHistoryListItemArrayWithBar: function () { // https://answers.sap.com/questions/204573/how-to-get-the-value-while-press-button-in-customl.html
			var items = [];
			var paintingHistory = this.getView().getModel().oData.paintingHistory;
			for (var i = 0; i < paintingHistory.length; i++) {
				var countHistory = i + 1;
				var customListItem = new sap.m.CustomListItem({press: "setMainImage"});
				var bar = new sap.m.Bar();
				var text = new sap.m.Text({text: countHistory + ". " + paintingHistory[i].artistName + " - " + paintingHistory[i].title});
				var button = new sap.m.Button({text: "View", type: "Emphasized"});
				bar.addContentLeft(text);
				bar.addContentRight(button);
				//customListItem.addContent(text,button);
				//customListItem.addContent(text);
				//customListItem.addContent(button);
				customListItem.addContent(bar);
				//new sap.m.StandardListItem({title: countHistory + ". " + paintingHistory[i].artistName + " - " + paintingHistory[i].title});
				items.push(customListItem);
			}
			return items;
		},

		openStyles1: function () {
			//this.loadPaintingStyleData("test", true); //cause error
			console.log("Opening menu style");
			var dialog = new sap.m.Dialog({
		...
		openHistoryDialog2: function () {
			console.log("Opening menu");
			//var menu = new sap.m.Menu("mainMenu", {title: "testMenu"});
			//var menu = new sap.ui.unified.Menu("mainMenu", {title: "testMenu"});
			//console.log(" menu", menu);
			//menu.openAsContextMenu();
			this.dialog = new sap.m.Dialog({
				title: 'Viewed Paintings',
				content: new sap.m.List({
					items: {
						path: '/paintingHistory',
		 ...
		 *
		 */
		loadPaintingStyleData: function (paintingStyle, featured) {
			// load_button
			// var oButton = this.byId("load_button");
			// console.log("oButton: " + oButton);
			var oModel = this.getView().getModel();
			//console.log("oModel", oModel);
			//console.log("oData", oModel.oData);
			//console.log("loadPaintingStyleData.");
			...
			var url = "https://cors.io/?https://www.wikiart.org/en/paintings-by-style/" + paintingStyle + "?" + featuredString + "json=2"; // use https://cors.io/ to circumvent No 'Access-Control-Allow-Origin' header is present on the requested resource problem
			// var url = "https://cors.io/?https://www.wikiart.org/en/paintings-by-style/" + paintingStyle + "?json=2&page=1"; // use https://cors.io/ to circumvent No 'Access-Control-Allow-Origin' header is present on the requested resource problem
			// var url = "https://cors.io/?https://www.wikiart.org/en/paintings-by-style/socialist-realism?json=2&page=1"; // use https://cors.io/ to circumvent No 'Access-Control-Allow-Origin' header is present on the requested resource problem
			...
			$.ajax({
				...
				success: function (data) {
					...
					jQuery.sap.log.info('Successfully retrieved data');
					oModel.setProperty("/paintingDataCurrentStyle", data);
					console.log("oModel after", oModel);
					//this.changeToRandomPicture(); // toDo: invoke this function here
					sap.m.MessageToast.show("Painting style loaded successfully", {
						duration: 1000,
						at: "right bottom"
					}); //toDo: add to i18n
				...
				error: function (error) {
					//sap.ui.getCore().getModel("controllers").getData().app.showErrorNotification();
					// sap.ui.getCore().getModel('controllers').getProperty('/app').showErrorNotification();
					jQuery.sap.log.error('Error on retrieving ressource -  for painting style: ' + paintingStyle);
					var dialog = new sap.m.Dialog("errorDialog", {
						title: 'Error',
						type: 'Message',
						state: 'Error',
						content: new sap.m.Text({
							text: 'The call to the API failed.'
						}),
						beginButton: new sap.m.Button({
							text: 'OK',
							press: function () {
					...
					dialog.open();
					// where are we here? in a different context
					// how to add a dialogue?
					var view = sap.ui.getCore().byId("mainview");
					var viewCopy = undefined;

					if (view === undefined) {
						viewCopy = sap.ui.xmlview("mainview", "sap.ui.demo.todo.view.App");
					}
					console.log("error case show view, viewCopy: ", viewCopy);
					var controller = sap.ui.getCore().byId("sap.ui.demo.todo").getController()
					console.log("error case show controller", controller);
					controller.showErrorNotification();
				}
			});
		},

		getIsFeaturedOld: function () {
			console.log("looking up featured: ");
			var comboBox = this.byId("comboBoxPaintingStyle");
			var changeToPaintingStyle = comboBox.getSelectedKey();

			var oModel = this.getView().getModel();
			var paintingStyles = oModel.oData.paintingStyles;
			console.log("paintingStyles", paintingStyles);
			console.log("paintingStyles typeof", typeof(paintingStyles));

			for (var i in paintingStyles) {
				console.log("this is number ", i, ": ", paintingStyles[i]);
				if (changeToPaintingStyle === paintingStyles[i].url) {
					console.log("I got paintingstyle featured, it is: ", paintingStyles[i].featured);
					return paintingStyles[i].featured;
				}
			}
			return false;
		},
